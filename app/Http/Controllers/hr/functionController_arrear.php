...
Public function EmployeeTypeList(){
	...
	,$qualication
	  ...
	  ,(case when LEFT(`tblper`.`fileNo`, 5)='SCN/P' then 1 else 2 end) asc
	  ,`tblper`.`fileNo`
	...
	Public function AllocationSource(){
		$List= DB::Connection("mysql2")->Select("SELECT * FROM `tblallocation_type` where status=1");
		return $List;
	}
	Public function BudgetType(){
		//$List= DB::Connection("mysql2")->Select("SELECT * FROM `tblcontractType` where status=1");
		$List= DB::Connection('mysql2')->table('tblcontractType')
		->where('status', 1)
		->get();
		return $List;
	}
	Public function EconomicHead($budgettype){
		$List= DB::Connection("mysql2")->Select("SELECT * FROM `tbleconomicHead` WHERE `contractTypeID`='$budgettype' and status=1");
		return $List;
	}
	Public function EconomicText($id){
	$data='Not Applicatable';
		$List= DB::Connection("mysql2")->Select("SELECT * FROM `tbleconomicCode` WHERE `ID`='$id'");
	...
	Public function EarningDeductionList($id){
		$list=DB::table('tblcvSetup')
	        ...
	        ->select('tblcvSetup.ID','Particular','description','status','economiccode')
	...
	Public function EconomicCode($allocationsource,$economichead){
		$List= DB::Connection("mysql2")->Select("SELECT * FROM `tbleconomicCode` WHERE `allocationID`='$allocationsource' and `contractGroupID`='$economichead' and status=1");
