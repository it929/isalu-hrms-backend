...
<table class="table table-condense table-responsive" border="1" cellpadding="4" cellspacing="0">
   ...
   @php
    ...
    <td align="right" >{{ $k++ }}</td>
      ...
      <td width="80" align="right" class="bg" style="background:#0cf; opacity:0.8"> 
       <?php
      if($reports->employment_type ==8 || $reports->employment_type==9 || $reports->employment_type==5)
      {
          $totalEarning = $reports->Bs + $reports->AEarn + $reports->SOT + $reports->PEC + $reports->hazard + $reports->callduty ;
      }
      else
      {
          $totalEarning = $reports->Bs + $reports->AEarn + $reports->SOT + $reports->PEC;
      }
      ?>
      
      <?php $earntotal += $totalEarning;?> <strong>{{number_format($totalEarning, 2, '.', ',')}} </strong>
      </td>
  ...
  <tr><td colspan="5" align="right"><strong>TOTAL</strong></td>
    ...
    <td align="right"><strong>{{ number_format($coopLoantotal, 2, '.', ',') }}</strong></td>
