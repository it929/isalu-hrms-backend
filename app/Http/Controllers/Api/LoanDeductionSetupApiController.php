...
class LoanDeductionSetupApiController extends Controller
{
    ...
    /**
     * GET /api/nextjs/payroll/loan-deduction-setups
     * Fetch existing loan setups.
     */
    public function index(Request $request)
    {
        try {
            ...
            $query = DB::table('loan_deduction_setups as lds')
                ->join('tblper as p', 'p.ID', '=', 'lds.staffId')
            ...
            if ($ctx['isSuperAdmin'] || $ctx['isAdminStaff'] || $ctx['isAuditStaff']) {
                ...
                $query->where('lds.staffId', $employee->ID);
            } else {
                $query->where('lds.id', 0); // fallback empty
            }

            $records = $query->orderBy('lds.id', 'desc')->get()->map(function ($row) {
            ...
            Log::error('LoanDeductionSetupApiController index: ' . $th->getMessage());
            return response()->json([
                'status' => 'error',
    ...
    /**
     * GET /api/nextjs/payroll/loan-deduction-setups/approved-amount/{staffId}
     * Get latest approved loan amount for a staff member.
     */
    public function getApprovedLoanAmount(Request $request, $staffId)
    {
        try {
            $ctx = $this->getUserContext($request);
            if (!$ctx) {
            ...
            return response()->json([
                'status' => 'success',
                'loan_amount' => $loan ? (float) $loan->loan_amount : 0.00,
                'balance' => $loan ? (float) $loan->balance : 0.00
            ]);
        } catch (\Throwable $th) {
            Log::error('LoanDeductionSetupApiController getApprovedLoanAmount: ' . $th->getMessage());
            return response()->json([
                'status' => 'error',
                'message' => $th->getMessage()
            ], 500);
    ...
    /**
     * POST /api/nextjs/payroll/loan-deduction-setups
     * Save or update a loan deduction setup.
     */
    public function store(Request $request)
    {
        try {
            ...
            if (!$ctx['isSuperAdmin'] && !$ctx['isAdminStaff']) {
                return response()->json([
                    'status' => 'error',
                    'message' => 'Access denied: Only administrators can configure loan deduction setups.'
                ], 403);
            }
            ...
            if ($id) {
                $exists = DB::table('loan_deduction_setups')->where('id', $id)->exists();
                if (!$exists) {
                    return response()->json(['status' => 'error', 'message' => 'Deduction setup not found.'], 404);
                }
                DB::table('loan_deduction_setups')->where('id', $id)->update($data);
                $message = 'Loan deduction setup updated successfully.';
            } else {
                $data['created_at'] = now();
                DB::table('loan_deduction_setups')->insert($data);
                $message = 'Loan deduction setup created successfully.';
            }
            ...
            Log::error('LoanDeductionSetupApiController store: ' . $th->getMessage());
            return response()->json([
                'status' => 'error',
                'message' => $th->getMessage()
            ], 500);
    ...
    /**
     * POST /api/nextjs/payroll/loan-deduction-setups/toggle/{id}
     * Toggle status.
     */
    public function toggleStatus(Request $request, $id)
    {
        try {
            ...
            DB::table('loan_deduction_setups')->where('id', $id)->update([
                'is_active' => $newStatus,
                'updated_at' => now()
            ]);
            ...
            Log::error('LoanDeductionSetupApiController toggleStatus: ' . $th->getMessage());
            return response()->json([
                'status' => 'error',
                'message' => $th->getMessage()
            ], 500);
    ...
    /**
     * DELETE /api/nextjs/payroll/loan-deduction-setups/{id}
    ...
    public function destroy(Request $request, $id)
    {
        try {
            ...
            DB::table('loan_deduction_setups')->where('id', $id)->delete();
            return response()->json([
                'status' => 'success',
                'message' => 'Loan deduction setup deleted successfully.'
            ]);
        } catch (\Throwable $th) {
            Log::error('LoanDeductionSetupApiController destroy: ' . $th->getMessage());
            return response()->json([
                'status' => 'error',
                'message' => $th->getMessage()
            ], 500);
    ...
    /**
     * GET /api/nextjs/payroll/loan-deduction-setups/template
    ...
    public function downloadTemplate(Request $request)
    {
        try {
            $headers = [
                ...
                'Content-Disposition' => 'attachment; filename="loan_setup_import_template.csv"',
                'Pragma'              => 'no-cache',
            ...
            $exampleRow = ['1024', '100000.00', '10', '10', '2026-06'];
            ...
            Log::error('LoanDeductionSetupApiController downloadTemplate: ' . $th->getMessage());
            return response()->json([
                'status' => 'error',
                'message' => $th->getMessage()
            ], 500);
    ...
    /**
     * POST /api/nextjs/payroll/loan-deduction-setups/import
    ...
    public function import(Request $request)
    {
        try {
            ...
            return response()->json([
                'status' => 'success',
                'message' => "Bulk import completed. {$importedCount} loan deduction setups imported successfully.",
                'warnings' => $warnings,
            ...
            DB::rollBack();
            Log::error('LoanDeductionSetupApiController import: ' . $th->getMessage());
            return response()->json([
                'status' => 'error',
                'message' => $th->getMessage()
            ], 500);
