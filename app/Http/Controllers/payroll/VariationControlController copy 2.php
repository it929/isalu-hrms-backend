    ...
    public function councilPayrollReport(Request $request)
{
    ...
    // Get payroll data with fixed division logic
    $data['payroll_detail'] = DB::table('tblpayment_consolidated')
        ->join('tblper', 'tblper.ID', '=', 'tblpayment_consolidated.staffid')
        ...
        ->where('tblpayment_consolidated.month', '=', $month)
        ->where('tblpayment_consolidated.year', '=', $year)
        ->where('tblpayment_consolidated.divisionID', '=', $division) // Simplified - always use the division
        ->where('tblpayment_consolidated.rank', '=', 2)
    ...
    // Check if payroll_detail is empty
    if ($data['payroll_detail']->isEmpty()) {
        return back()->with('error', 'No payroll data found for the selected criteria. Please check the month, year, and division.');
    }

    // Only call DynamicEaringDeduction if we have data
    try {
