...
.pr
        ...
        .table tr .bg
        {
            background:#0cf; font-weight:700;
        }
...
<table class="table table-condense table-responsive" border="1" cellpadding="4" cellspacing="0">
  <tr class="th-row">
    ...
    @if($count_sot > 0)
    <td colspan="8" align="center" valign="top"><strong>EARNINGS</strong></td>
    @else
    <td colspan="7" align="center" valign="top"><strong>EARNINGS</strong></td>
    @endif
    
    @if($alhisan > 0)
    <td colspan="9" align="center" valign="top"><strong>DEDUCTIONS</strong></td>
    @else
    <td colspan="8" align="center" valign="top"><strong>DEDUCTIONS</strong></td>
    @endif
  ...
  <tr>
    ...
    <td align="center" valign="middle"><strong>MEDICAL <br/> ALLOWANCE</strong></td>
    @if($count_sot > 0)
    ...
    <td width="52" align="center" valign="middle"><strong>SEC ALL</strong></td>
    <td width="75" align="center" valign ="middle"><strong> JUSU/ <br/> PECULIAR </strong></td>
    ...
    <td width="50" align="center" valign="middle"><strong>VOLUNTARY <br/> PENSION</strong></td>
    ...
    @if($alhisan > 0)
    <td width="80" align="center" valign="middle"><strong>AL-IHSAN</strong></td>
    @endif
    <td width="85" align="center" valign="middle"><strong>SALARY ADVANCE</td>
    <td width="85" align="center" valign="middle"><strong>REFUNDS</td>
   ...
   @php
      ...
      //*****************************SORTING WITH MONTHS AND YEARS PROVIDED ****************/
        ...
        $refunds = DB::table('tblotherEarningDeduction')
            ...
            ->whereIn('tblotherEarningDeduction.month', $monthsToSearch)
            //->where('tblotherEarningDeduction.year', '=', $year)
            ->whereBetween('tblotherEarningDeduction.year', [$year, $toYear])
            ...
            //->where('tblpayment_consolidated.bank', '=',$reports->bankid)
            ->where('tblpayment_consolidated.staffid', '=', $reports->staffid)
            ->groupBy('tblotherEarningDeduction.staffid')
            ->sum('tblotherEarningDeduction.amount');
            
            
    
       $coopSaving = DB::table('tblotherEarningDeduction') ->where('staffid', '=', $reports->staffid)
                        //->where('month', '=', $month)
                        ->whereIn('month', $monthsToSearch)
                        //->where('year', '=', $year)
                        ->whereBetween('year', [$year, $toYear])
                        ->where('CVID', '=', 15)
                        ->groupBy('staffid')
                        ->first();
     
      $coopLoan = DB::table('tblotherEarningDeduction') ->where('staffid', '=', $reports->staffid)
                        //->where('month', '=', $month)
                        ->whereIn('month', $monthsToSearch)
                        //->where('year', '=', $year)
                        ->whereBetween('year', [$year, $toYear])
        ...
        $salAdvance = DB::table('tblotherEarningDeduction') ->where('staffid', '=', $reports->staffid)
                        //->where('month', '=', $month)
                        ->whereIn('month', $monthsToSearch)
                        //->where('year', '=', $year)
                        ->whereBetween('year', [$year, $toYear])
      ...
      $overtime = DB::table('tblotherEarningDeduction') ->where('staffid', '=', $reports->staffid)
                        //->where('month', '=', $month)
                        ->whereIn('month', $monthsToSearch)
                        //->where('year', '=', $year)
                        ->whereBetween('year', [$year, $toYear])
      ...
      $secAllow = DB::table('tblotherEarningDeduction') ->where('staffid', '=', $reports->staffid)
                        //->where('month', '=', $month)
                        ->whereIn('month', $monthsToSearch)
                        //->where('year', '=', $year)
                        ->whereBetween('year', [$year, $toYear])
      ...
      $mth = array("JANUARY", "FEBRUARY", "MARCH", "APRIL", "MAY", "JUNE", "JULY", "AUGUST"); 
      ...
      else
      {
       ...
       $vpen = DB::table('tblotherEarningDeduction')->where('staffid', '=', $reports->staffid)
                        //->where('month', '=', $month)
                        ->whereIn('month', $monthsToSearch)
                        //->where('year', '=', $year)
                        ->whereBetween('year', [$year, $toYear])
       ...
       {
       $vpenAmount = $vpen->amount;
       }
       
       $alhisans = DB::table('tblotherEarningDeduction') ->where('staffid', '=', $reports->staffid)
                        //->where('month', '=', $month)
                        ->whereIn('month', $monthsToSearch)
                        //->where('year', '=', $year)
                        ->whereBetween('year', [$year, $toYear])
        ...
        $alhisansLoan = DB::table('tblotherEarningDeduction')->where('staffid', '=', $reports->staffid)
                        //->where('month', '=', $month)
                        ->whereIn('month', $monthsToSearch)
                        //->where('year', '=', $year)
                        ->whereBetween('year', [$year, $toYear])
       ...
       {
       $alhisanAmount = $alhisans->amount;
       }
       if($alhisansLoan == '')
       {
        $alhisansLoanAmount = 0;
       }
       else
       {
       $alhisansLoanAmount = $alhisansLoan->amount;
       }
        //$alhisanAmount = 0;
        ...
        //get total gross
        $getTotalGrossEmolument = $reports->Bs + $reports->AEarn + $medAll_ + $ov + $sa + $reports->PEC; // + $reports->SOT
    ...
    <td align="right" >{{ $k++ }}</td>
      ...
      <td width="23" align="center" valign="middle">@if($reports->employment_type == 3)  @else {{ $reports->grade }} @endif</td>
      <td width="23" align="center" valign="middle"> @if($reports->employment_type == 3)  @else {{$reports->step}}  @endif</td>
      ...
      @else
        <td width="66" align="right"><?php $e_arrearstotal += $reports->AEarn;?>
            <a class="hidden-print" href="{{url("/con-payrollReport/arrears/$reports->courtID/$reports->staffid/$reports->year/$reports->month")}}" target="_blank">{{number_format($reports->AEarn, 2, '.', ',')}}</a>
            <span class="pr">{{number_format($reports->AEarn, 2, '.', ',')}}</span>
        </td>
      @endif
      
      <td align="right">
            @if($reports->employment_type == 5 || $reports->employment_type == 8 || $reports->employment_type == 9)
                <?php $medAllowanceTotal += ($reports->callduty + $reports->hazard);?>
                {{ number_format(($reports->callduty + $reports->hazard ),2)}}
                @php $medAll_ = ($reports->callduty + $reports->hazard); @endphp
            @else
                    @php $medAll_ = 0.00; @endphp
                    0.00
            @endif
      </td>
      ...
      @endif
      <td width="38" align="right"><?php $overtimeTotal += $ov; ?> {{ number_format($ov,2)}}</td>
      <td width="38" align="right"><?php $secAllowTotal += $sa; ?>{{ number_format($sa,2)}}</td>
      <td width="41" align="right" valign="middle"><?php $pectotal += $reports->PEC; /*($reports->employment_type == 5)?0: $reports->PEC;*/?> 
          <!--@if($reports->employment_type == 5 || $reports->employment_type == 8 || $reports->employment_type == 9)
          0.00
          @else
     ...
     @if($year <= 2020 && in_array($month, $mth))
      <td width="80" align="right" class="bg" style=""> 
            <?php $earntotal += $reports->TEarn - ($reports->OEarn - ($reports->callduty + $reports->hazard + $reports->SOT));?> 
            <strong>{{number_format($reports->TEarn-($reports->OEarn - ($reports->callduty + $reports->hazard + $reports->SOT)), 2, '.', ',')}} </strong>
      </td>
      
      @else
      <td width="80" align="right" class="bg" style=""> 
            <!--Get new Gross-->
            <?php  $earntotal += $reports->gross; ?> {{ number_format($reports->gross,2) }}
              {{--  $earntotal += $getTotalGrossEmolument;    number_format($getTotalGrossEmolument,2) --}}
      </td>
     
      @endif
      <!--<td width="38" align="right"><?php $e_otherstotal += $reports->OEarn - ($reports->callduty + $reports->hazard);?>
     
          {{ number_format($reports->OEarn - ($reports->callduty + $reports->hazard),2)}}
      ...
      <td width="74" align="right"><?php $vpentotal += $vpenAmount;?> {{number_format($vpenAmount, 2, '.', ',')}}</td>
      ...
      @if($alhisan > 0)
        <td width="74" align="right"><?php $alhisanTotal += $alhisanAmount + $alhisansLoanAmount;?> {{number_format($alhisanAmount + $alhisansLoanAmount, 2, '.', ',')}}</td>
        @php $getAlhisan = $alhisanAmount + $alhisansLoanAmount; @endphp
      @else 
            @php $getAlhisan = 0.00; @endphp
      @endif
      ...
      <td width="85" align="right"><?php $totalRefunds += $refunds ?> {{ number_format($refunds, 2) }}</td>
      
       
            
      <td width="82" align="right" class="bg" style="">
          
          {{-- $deducttotal += $reports->TD; --}}
          {{-- number_format($allDeducts, 2, '.', ',') --}}
          <!--{{-- ($reports->TAX + $reports->PEN + $vpenAmount + $reports->UD + $reports->NHF + ($alhisanAmount + $alhisansLoanAmount) + ((count($salAdvance) ==0) ? 0 : $salAdvance->amount) + ((count($coopSaving) ==0 ) ? 0 : ($coopSaving->amount)) + ((count($coopLoan) ==0 ) ? 0 : ($coopLoan->amount)) ); --}}-->
         
          <strong> <?php $deducttotal += $reports->TD; ?>  {{ number_format($reports->TD, 2, '.', ',') }}  </strong>
      ...
      <td width="82" align="right">
          ...
          {{-- number_format(($getTotalGrossEmolument - $allDeducts), 2) --}}
          
           <?php $totalNetEmolu += $reports->NetPay; ?> {{ number_format(($reports->NetPay), 2) }} 
  ...
  <tr><td colspan="5" align="right"><strong>TOTAL</strong></td>
    ...
    <td align="right"><strong>{{ number_format($bstotal, 2, '.', ',') }}</strong></td>
        ...
        <td align="right"><strong>{{ number_format($medAllowanceTotal, 2, '.', ',') }} </strong></td>
   
     @if($count_sot > 0)
    <td align="right"><strong>{{ number_format($totalSot, 2, '.', ',') }}</strong></td>
    @endif
    <td align="right"><strong>{{ number_format($overtimeTotal, 2, '.', ',') }}</strong></td>
    <td align="right"><strong>{{ number_format($secAllowTotal, 2, '.', ',') }}</strong></td>
    ...
    <td align="right"><strong>{{ number_format($vpentotal, 2, '.', ',') }} </strong></td>
    ...
    <td align="right"><strong>{{ number_format($coopLoantotal + $coopSavingtotal, 2, '.', ',') }}</strong></td>
    @if($alhisan > 0)
    <td align="right"><strong>{{ number_format($alhisanTotal, 2, '.', ',') }}</strong></td>
    @endif
    
    <td align="right"><strong>{{ number_format($salAdvancetotal, 2, '.', ',') }}</strong></td>
    
    <td align="right"><strong>{{ number_format($totalRefunds, 2) }}</strong></td>
