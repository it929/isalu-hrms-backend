...
export default function DepartmentPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { department, court } = formData;
    ...
    try {
      ...
      const payload = {
        department,
        court: courtInfo.courtstatus === 0 ? courtInfo.courtid : court
      };
            ...
            <table className={styles.table}>
              ...
              <tbody>
                {departmentList.length === 0 ? (
                  <tr>
                    <td colSpan={courtInfo.courtstatus === 1 ? 4 : 3} style={{ textAlign: 'center', padding: '3rem 0' }}>
                  ...
                  departmentList.map((dept, index) => (
                    <tr key={dept.id}>
                      <td>{index + 1}</td>
                      {courtInfo.courtstatus === 1 && <td>{dept.court_name || 'N/A'}</td>}
