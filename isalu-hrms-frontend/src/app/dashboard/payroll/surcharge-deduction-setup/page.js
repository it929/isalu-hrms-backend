...
export default function SurchargeDeductionSetupPage() {
  // UI & Loading States
  ...
  // Fetch dynamic surcharge setups list
  const fetchSetups = useCallback(async (silent = false) => {
    const cacheKeySetups = 'hrms_surcharge_setups_cache';
    let hasCache = false;
    ...
    const headers = buildHeaders();
    try {
      const res = await axios.get(`${API_BASE}/payroll/surcharge-deduction-setups`, { headers });
      ...
      showToast('Failed to retrieve surcharge setups.', 'error');
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_BASE}/payroll/surcharge-deduction-setups`, payload, {
        headers: buildHeaders()
      });

      if (res.data.status === 'success') {
        showToast(res.data.message || 'Surcharge configuration saved successfully.');
        handleClearForm();
        fetchSetups(true);
      } else {
  ...
  const handleToggleStatus = async (id) => {
    try {
      const res = await axios.post(`${API_BASE}/payroll/surcharge-deduction-setups/toggle/${id}`, {}, {
        headers: buildHeaders()
      });
      if (res.data.status === 'success') {
  ...
  const handleDelete = (id) => {
    setConfirmAction({ type: 'delete', id, label: 'Delete Surcharge Setup' });
  };
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (type === 'delete') {
        const res = await axios.delete(`${API_BASE}/payroll/surcharge-deduction-setups/${id}`, {
          headers: buildHeaders()
        });
        if (res.data.status === 'success') {
  ...
  const uploadFile = async (file) => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/payroll/surcharge-deduction-setups/import`, formData, {
        headers: {
          ...buildHeaders(),
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h1 className={styles.title}>Surcharge Deduction Setup</h1>
        <p className={styles.subtitle}>Configure surcharge deductions from employee salaries, supporting both one-time deductions and monthly spreading, as well as bulk spreadsheet importing.</p>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.2fr) minmax(0, 0.8fr)', gap: '1.5rem', marginBottom: '1.5rem' }}>
          ...
          <div className={styles.card} style={{ marginBottom: 0 }}>
            <div className={styles.cardHeader}>
              <h2 className={styles.cardTitle}>{editSetupId ? 'Modify Surcharge Setup' : 'Create Surcharge Setup'}</h2>
            </div>
              ...
              <form onSubmit={handleSubmit}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                    ...
                    {/* Total Amount */}
                    <div className={styles.formGroup}>
                      <label className={styles.label}>Total Surcharge Amount (₦) *</label>
            ...
            <div className={styles.cardBody} style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              ...
              <div style={{ marginTop: '1.25rem', fontSize: '0.825rem', color: '#64748b', lineHeight: '1.4' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.35rem' }}>
                  ...
                  <a
                    href={`${API_BASE}/payroll/surcharge-deduction-setups/template`}
                    download="surcharge_import_template.csv"
                    style={{
        ...
        <div className={styles.cardBody} style={{ padding: 0 }}>
          {loading ? (
            ...
            <div className={styles.emptyState}>
              ...
              <p>No surcharge configurations found.</p>
            </div>
          )}
          ...
          <div className={styles.modalBox}>
            <div className={styles.confirmBox}>
              ...
              <p className={styles.confirmMsg}>
                Are you sure you want to delete this surcharge configuration? This action cannot be undone.
              </p>
