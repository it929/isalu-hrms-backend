      ...
      <div class="row">
        ...
        <div align="right" class="col-xs-6" style="padding-top:90px; padding-right:40px;">
          <table > 
            ...
            <!--COA/ZENITH/{{$nmonth = date("m", strtotime($month))}}/BAT-@if(count($bat) > 0) {{$bat->batNo}} @else @endif  <br/>-->
 ...
 @endforeach
  ...
  <div style="width:100%;float:left;">
      <div class="" style="width:80%; margin:10px auto">
          Please credit the account(s) of the under listed beneficiary(s) and debit our account above with: (&#8358;)<b> <span id="grandTotalAmount"></span> </b><br>
          <span id="result">                    
      </div>
  </div>
          ...
          <td colspan="2">
            <table class="table table-responsive table-bordered" id="tableData">
                  <tr class="tblborder">
                    ...
                    <td class="tblborder"><div align="center"><strong>BENEFICIARY</strong></div><div align="center"></div>            
                 ...
                 <?php 
                  ...
                  <tr class="tblborder">
                    ...
                    <td class="tblborder">  @if($reports->remarks!=null && $reports->month == session('month') ) {{$reports->remarks}} @else {{session('month')}}  {{session('year') }} Staff Salary @endif </td>
                  </tr>
                   <?php $counter=$counter+1;   ?>
                  @endif
                  <?php 
                 ...
                 <?php 
                  ...
                  <tr>
                    <td class="tblborder"><strong>SN</strong></td>
                    <td class="tblborder" colspan="2"><strong>BENEFICIARIES</strong></td>
                    <td class="tblborder"><strong>BANK NAME</strong></td>
                    <td class="tblborder"><strong>ACCOUNT NUMBER</strong></td>
                    <td class="tblborder"><strong>S/CODE</strong></td>
                    <td class="tblborder"><strong>AMOUNT</strong></td>
                    <td class="tblborder"><strong>MONTH</strong></td>  
                  </tr>

                  <tr>
                    <td class="tblborder">1</td>
                  ...
                  <tr>
                    ...
                    <td class="tblborder" colspan=2>FCT-IRS</td>
                    <td class="tblborder"> </td>
                    <td class="tblborder"> </td>
                    <td class="tblborder"></td>
                  ...
                  <tr>
                    ...
                    <td class="tblborder" colspan=2>NHF</td>
                    <td class="tblborder"> </td>
                    <td class="tblborder"></td>
                    <td class="tblborder"></td>
                    <td class="tblborder">@if(session('month') == 'OCTOBER' && session('year') == 2019) {{'NILL'}} @else {{number_format($nhf,2)}} @endif</td>
                    <td class="tblborder">{{session('month')}}  {{session('year')}}  DEDUCTION</td>  
                  </tr>

                  @php $totalCVAmount = 0.0; @endphp
                  @if(isset($staffDeductionElement) && $staffDeductionElement)
                    @foreach ($staffDeductionElement as $keyItem=>$cvItem)
                    @php //$totalCVAmount += $cvItem->staffEarnings; @endphp
                      <tr>
                        <td class="tblborder">{{$keyItem + 4}}</td>
                        <td class="tblborder" colspan=2>
                          <a href="javascript:;" onclick="getAccountDetailsFunction('@php echo $cvItem->cvsetupID @endphp', '@php echo $cvItem->account_name @endphp', '@php echo $cvItem->account_number @endphp', '@php echo $cvItem->description @endphp')"
                            data-toggle="modal" data-target="#updateAccountModal">
                            {{strtoupper($cvItem->description)}}
                          </a>
                        </td>
                        <td class="tblborder"><span id="accNameTable{{$cvItem->cvsetupID}}">{{$cvItem->account_name}}</span></td>
                        <td class="tblborder"><span id="accNumTable{{$cvItem->cvsetupID}}">{{$cvItem->account_number}}</span></td>
                        <td class="tblborder"></td>
                        <td class="tblborder">@php $totalCVAmount += $cvItem->staffEarnings; @endphp {{number_format($cvItem->staffEarnings, 2)}}</td> 
                        <td class="tblborder">{{session('month')}}  {{session('year')}}  DEDUCTION</td>  
                    </tr>
                    @endforeach
                  @endif
                  ...
                  <tr>
                    ...
                    <td colspan ="3" class="tblborder"><strong>  {{number_format(($totalCVAmount + $nhf + $taxSum + $unionSum),2)}} </strong</td>
                  </tr>
                  <tr>
                    <td colspan ="5"class="tblborder"><strong>GRAND TOTAL</></td>
                      @php $grandTotalSum = number_format($totalCVAmount + $nhf + $taxSum + $unionSum + $sum ,2); @endphp
                    <td colspan ="3" class="tblborder"><strong>  {{number_format($totalCVAmount + $nhf + $taxSum + $unionSum + $sum ,2)}} </strong</td>
      ...
      <div class="modal-body">
        ...
        <div class="row">
          <div class="col-md-6">
            ...
            <input type="text" name="accountNumber" id="accountNumberModal" class="form-control"/>
            <input type="hidden" name="cvID" id="cvIDModal" />
          </div>
          <div class="col-md-6">
            <label>Account Name</label>
            <input type="text" name="accountName" id="accountNameModal" class="form-control"/>
          </div>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
        <button type="button" id="updateNowBtn" class="btn btn-success" data-dismiss="modal">Update</button>
      </div>
    </div>
  </div>
</div>
...
$.ajax({
  ...
  $("#grandTotalAmount").html("<?php echo $grandTotalSum ?>");
</script>

<script type="text/javascript">
  var amount = "";
  var amount = "<?php echo $grandTotalSum; ?>";
