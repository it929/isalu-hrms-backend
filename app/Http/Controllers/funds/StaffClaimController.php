...
class StaffClaimController extends BasefunctionController
{
   ...
   //controllers for normal staff
    ...
    public function pushClaimHod(Request $request)
    {
        ...
        if($getTotalStaffAmount<1) return back()->with('error','Total amount must be greater than 0.00');
        ...
        $allbeneficiary= DB::table('tblselectedstaffclaim')->leftJoin('tblStaffInformation', 'tblStaffInformation.staffID', '=', 'tblselectedstaffclaim.staffID')
        ->where('claimID', $claimID)->select('tblselectedstaffclaim.*','tblStaffInformation.full_name')->get();
        
        foreach ($allbeneficiary as $ben) 
    	    {
    		 if($ben->staffamount<1) return back()->with('error','Total amount alloted to '. $ben->full_name. ' must be greater than 0.00');
    		}
	        ...
	        ]);
	   {
    	...
    	if($success){
    	    $userFileDetails = DB::table('tblStaffInformation')->where('userID', Auth::user()->id)->first();
	        	...
	        	'contractValue' => DB::table('tblselectedstaffclaim')->where('claimID', $claimID)->sum('staffamount'),
	        	'companyID' => 13, 
	        	'beneficiary' => $beneficiary,
	        	'dateAward' => date('Y-m-d'), 
	        	'approvedBy' => DB::table('users')->where('id', Auth::user()->id)->value('name'),
	        	'createdby'=>$claimDetails->user,
	        	'approvalStatus' => 1, 
	           ...
	           $taskscheduled=$this->UpdateAlertTable("Payment approval review",'procurement/approve','',($submitTo ? $submitTo : 'DFA'),'tblcontractDetails',$lastid,1);
        ...
        return redirect()->back()->with('message', 'Record  forwarded successfully.');
    ...
    {	
    	...
    	if(DB::table('tblclaim')->where('ID', $claimID)->first())
    	{
    		...
    		if($office == 'ES'){ 
	            ...
	            //
	            $success = DB::table('tblcontractDetails')->insertGetId([
	        	'fileNo' => DB::table('tblclaim')->where('ID', $claimID)->value('claimFileNo'),
	        	'procurement_contractID' => $claimID, 
	        	'staffid' => $claimDetails->user,
	        	'ContractDescriptions' => $claimDetails->details,
    		    ...
    		    $taskscheduled=$this->UpdateAlertTable("Payment approval review",'procurement/approve','',($submitTo ? $submitTo : 'DFA'),'tblcontractDetails',$success,1);
    		    if($success){
    		        if($office == 'ES'){
    ...
    public function uploadAttachStaffClaimFile(Request $request)
    {
        ...
        if($file or !empty($file) or $file != null) {
            ...
            if (DB::table('tblclaim')->where('ID', $claimID)->orWhere('status', 3)->orWhere('status', 2)->first())
            {
                ...
                $fileNewName = $claimID . '-' .$this->randomNo() . '.' . $fileOriginalExtension;
