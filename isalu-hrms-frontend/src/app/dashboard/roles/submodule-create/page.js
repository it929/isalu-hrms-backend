...
export default function CreateSubModulePage() {
  ...
  const fetchSubModules = useCallback(async (pageNum = 1, filterModId = '') => {
    setLoading(true);
  ...
  useEffect(() => {
    fetchSubModules(1, selectedFilterModule);
  ...
  const handleAddSubModuleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        setRank('');
        setSelectedModule('');
        fetchSubModules(1, selectedFilterModule); // reload list
      } else {
  ...
  const handleEditSubModuleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        closeEditModal();
        fetchSubModules(page, selectedFilterModule); // reload current page
      } else {
  ...
  const handleDeleteSubModule = async () => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        closeEditModal();
        fetchSubModules(1, selectedFilterModule); // reload first page
      } else {
