...
class ConPayrollReportControllerRevamp extends ParentController
{
  /**
...
public function create()
    {
        ...
        return view('payrollReport_con.index-revamp', $data);
    }
  ...
  public function getBank(Request $request)
   ...
   // Recieves an ajax request with a division ID and returns all  bank under that division
    ...
    public function payrollComments($division, $year, $month)
    {
  ...
  $payroll_detail = DB::table('tblpayment_consolidated')
    ...
    //Earnings
    $data['totalEarningCount'] = DB::table('tblotherEarningDeduction')
      ...
      ->select('description', 'tblotherEarningDeduction.CVID as CVID')
      ->get();
      
    //Deductions
    $data['totalDeductionCount'] = DB::table('tblotherEarningDeduction')
      ->Join('tblcvSetup', 'tblotherEarningDeduction.CVID', '=', 'tblcvSetup.ID')
      ->where('tblotherEarningDeduction.year', $year)
      ->where('tblotherEarningDeduction.month', $month)
      ->where('tblotherEarningDeduction.divisionID', $division ? '=' : '<>', $division)
      ...
      ->orderBy('tblcvSetup.rank')
      ->groupBy('tblotherEarningDeduction.CVID')
      ->select('description', 'tblotherEarningDeduction.CVID')
      ->get();
    ...
    foreach ($payroll_detail as $key => $value) {
      ...
      //Get staff
        ...
        //   }
        ///============================
        foreach($data['totalEarningCount'] as $getCvID){ //[$listOfStaff][$getCvID->CVID]
            $staffEarnElement = DB::table('tblotherEarningDeduction')
              ->where('tblotherEarningDeduction.year', $year)
              ->where('tblotherEarningDeduction.month', $month)
              ->where('tblotherEarningDeduction.divisionID', $division ? '=' : '<>', $division)
              ->where('tblotherEarningDeduction.particularID', 1)
              ->where('tblotherEarningDeduction.amount', '<>', 0)
              ->where('tblotherEarningDeduction.staffid', $value->staffid)
              ->where('tblotherEarningDeduction.CVID', $getCvID->CVID)
              ->first([DB::raw("SUM(tblotherEarningDeduction.amount) as staffEarnings")]);
              $getEarnAmount[$value->staffid][$getCvID->CVID] = $staffEarnElement->staffEarnings;
        }
        
    }
    dd($getEarnAmount);
    $data['staffEarnElementByStaffId'] = $getEarnAmount;
    $data['payroll_detail'] = $getPayroll_detail;
    
    
    // $getdynamicData = $this->DynamicEaringDeduction($value->staffid, $year, $month, $division, $bankID);
    // $data['staffEarnElement'] = $getStaffMonthEarnAmount;
    // //$data['staffDeductionElement'] = $getdynamicData['staffDeductionElement'];
    // $data['totalEarningCount'] = $getdynamicData['totalEarningCount'];
    // $data['totalDeductionCount'] = $getdynamicData['totalDeductionCount'];
    ...
    return view('payrollReport_con.summary-revamp', $data);
    //===================== query ends ====================
  ...
  public function DynamicEaringDeduction($listOfStaff, $year = null, $month = null, $division = null, $bankID = null, $totalEarningCount,  $totalDeductionCount)
  {
    //=========================START===================================
    // $getStaffMonthEarn = [];
    ...
    $staffEarnElement = [];
      ...
      foreach($totalEarningCount as $getCvID){ //[$listOfStaff][$getCvID->CVID]
            $staffEarnElement[] = DB::table('tblotherEarningDeduction')
              //->Join('tblcvSetup', 'tblotherEarningDeduction.CVID', '=', 'tblcvSetup.ID')
              ->where('tblotherEarningDeduction.year', $year)
              ->where('tblotherEarningDeduction.month', $month)
              ...
              ->where('tblotherEarningDeduction.particularID', 1)
              ->where('tblotherEarningDeduction.amount', '<>', 0)
              ->where('tblotherEarningDeduction.staffid', $listOfStaff)
              ->where('tblotherEarningDeduction.CVID', $getCvID->CVID)
              //->orderBy('tblcvSetup.rank')
            //   ->groupBy('tblotherEarningDeduction.staffid')
            //   ->groupBy('tblotherEarningDeduction.CVID')
              //->select(DB::raw("SUM(tblotherEarningDeduction.amount) as staffEarnings"), 'amount', 'tblotherEarningDeduction.CVID', 'tblcvSetup.ID as cvsetupID', 'tblotherEarningDeduction.ID', 'rank', 'tblcvSetup.particularID', 'tblotherEarningDeduction.staffid')
              ->first([DB::raw("SUM(tblotherEarningDeduction.amount) as staffEarnings"), 'amount']);
        }
        
      
      //$results = DB::select(DB::raw('SELECT column1, column2, COUNT(*) as count FROM your_table GROUP BY column1, column2'));
    ...
    //   ->groupBy('tblotherEarningDeduction.staffid')
    //   ->groupBy('tblotherEarningDeduction.CVID')
    //   ->select(DB::raw("SUM(tblotherEarningDeduction.amount) as staffDeductions"), 'description', 'amount', 'tblotherEarningDeduction.CVID', 'tblcvSetup.ID as cvsetupID', 'tblotherEarningDeduction.ID', 'rank', 'tblcvSetup.particularID', 'tblotherEarningDeduction.staffid')
    //   ->get();
    ...
    // }
        ...
        $data['getStaffMonthEarnAmount'] = $staffEarnElement;
