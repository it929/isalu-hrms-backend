...
class ActiveMonthController extends ParentController
{
    ...
    public function create()
    {
        ...
        $data['activemonth'] = DB::table('tblactivemonth')
            ...
            ->get();
        //dd($data['activemonth']);
        return view('payroll.activeMonth.active_month', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($count == 1) {
            ...
            return redirect("/activeMonth/create1")->with('message', ' operation was successfully performed');
        } elseif ($count == 0) {
            $update = DB::table('tblactivemonth')
            ...
            $this->addLog(" active month set  to " . $month . "/" . $year . " for " . $courtname->court_name . " Court  ");
            // return redirect("/activeMonth/create1")->with('message', ' operation was successfully performed');
            return redirect()->back()->with('message', 'Active month set successfully!');
        } else {
            // return redirect("/activeMonth/create1")->with('error_message', ' operation h failed. Please try again');
            return redirect()->back()->with('message', 'operation  failed. Please try again!');
        }
    }
}
