    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($ctx['isSuperAdmin'] || $ctx['isAdminStaff'] || $ctx['isAuditStaff']) {
                ...
                $query->where('css.staffId', $employee->ID);
            } else {
                $query->where('css.id', 0); // fallback empty
            }
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (!$ctx['isSuperAdmin'] && !$ctx['isAdminStaff']) {
                return response()->json([
                    'status' => 'error',
                    'message' => 'Access denied: Only administrators can configure cooperative savings setups.'
                ], 403);
            }
