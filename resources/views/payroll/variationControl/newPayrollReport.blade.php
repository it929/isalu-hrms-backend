...
<table class="table table-condense table-responsive" border="1" cellpadding="4" cellspacing="0">
  <tr class="th-row">
   <td width="44" rowspan="2" align="center" valign="middle"><strong>SN</strong></td>
    ...
    @else
    <td colspan="7" align="center" valign="top"><strong>DEDUCTIONS</strong></td>
    @endif
  ...
  @foreach ($payroll_detail as $reports)
   ...
   @php
     ...
     $coopSaving = DB::table('tblotherEarningDeduction')->where('staffid','=',$reports->staffid)->where('month','=',$month)->where('year','=',$year)->where('CVID','=',15)->first();
      ...
      $secAllow = DB::table('tblotherEarningDeduction')->where('staffid','=',$reports->staffid)->where('month','=',$month)->where('year','=',$year)->where('CVID','=',17)->first();
      ...
      else
      {
       ...
       $vpen = DB::table('tblotherEarningDeduction')->where('staffid','=',$reports->staffid)->where('month','=',$month)->where('year','=',$year)->where('CVID','=',27)->first();
       ...
       $alhisans = DB::table('tblotherEarningDeduction')->where('staffid','=',$reports->staffid)->where('month','=',$month)->where('year','=',$year)->where('CVID','=',31)->first();
        $alhisansLoan = DB::table('tblotherEarningDeduction')->where('staffid','=',$reports->staffid)->where('month','=',$month)->where('year','=',$year)->where('CVID','=',33)->first();
       if($alhisans == '')
       {
        $alhisanAmount = 0;
       }
    ...
    <td align="right" >{{ $k++ }}</td>
     ...
     @if($year <= 2020 && in_array($month, $mth))
      ...
      <?php $earntotal += $reports->TEarn - ($reports->OEarn - ($reports->callduty + $reports->hazard + $reports->SOT));?> <strong>{{number_format($reports->TEarn-($reports->OEarn - ($reports->callduty + $reports->hazard + $reports->SOT)), 2, '.', ',')}} </strong>
      </td>
      
      @else
      <td width="80" align="right" class="bg" style=""> 
       <?php $earntotal += $reports->gross + $reports->SOT; ?>{{number_format($reports->gross + $reports->SOT,2)}}
      </td>
      ...
      <td width="82" align="right" class="bg" style=""><?php $deducttotal += $reports->TD;?> <strong> {{number_format($reports->TD, 2, '.', ',')}}</strong></td>
