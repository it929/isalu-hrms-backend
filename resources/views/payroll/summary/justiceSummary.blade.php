...
<div align="center" class="marginTableHeading"><strong><span class="style2">
  {{-- <h3>
    <br />
      <div>{{ strtoupper(isset($divisionName) && $divisionName ? $divisionName : "Abuja") }}</div>
  </h3> --}}
  <h4>ISALU HOSPITAL LIMITED</h4>
  {{-- <h5>PAYMENT VOUCHER EMOLUMENTS - SENIOR OFFICERS</h5> --}}
  <h5>PAYMENT VOUCHER EMOLUMENTS - JUSTICE</h5>
</span>
        </strong></div>
<br />
<?php
  ...
  @foreach ($summary_detail as $reports)
    <?php
        ...
        $deducttotal =  $taxtotal + $pentotal + $nhftotal + $uniontotal;  //$reports->TD;
...
@endif REGULAR ALLOWANCE</p>
  <table class="table table-condense table-responsive" width="1586" border="1" align="center" cellpadding="0" cellspacing="0">
    ...
    <td colspan="6" align="center" valign="top"><strong>EARNINGS</strong></td>
  ...
  <tr>
    ...
    <td width="75" align="center" valign ="middle"><strong> PECULIAR/ALLOWANCE </strong></td>
    <td width="75" align="center" valign ="middle"><strong> PECULIAR FG </strong></td>
    ...
    @php $totalDeductAmount = 0.0; @endphp
    @if(isset($staffDeductionElement) && $staffDeductionElement)
      @foreach($staffDeductionElement as $elementDeduct)
          @php $totalDeductAmount += $elementDeduct->deductionAmount; @endphp
          <td width="80" align="center" valign="middle"><strong>  {{strtoupper($elementDeduct->description)}}</strong></td>
      @endforeach
    @endif
  </tr>
  ...
  <tr>
    ...
    <td align="right"><strong>{{ number_format(($bstotal + $e_arrearstotal + $e_otherstotal + $pectotal + $pectotalFG), 2) }}</strong></td>
    ...
    @if(isset($staffDeductionElement) && $staffDeductionElement)
      @foreach($staffDeductionElement as $elementDeduct)
          <td align="right"><strong>{{ number_format($elementDeduct->deductionAmount, 2) }}</strong></td>
      @endforeach
    @endif
    <td align="right"><strong>{{ number_format(($deducttotal + $totalDeductAmount), 2) }}</strong></td>
    {{-- <td align="right"><strong>{{ number_format((($netpaytotal + $deducttotal + $totalDeductAmount)-($pectotal + $pectotalFG)) - ($deducttotal + $totalDeductAmount), 2) }}</strong></td> --}}
    ...
    @for ($i = 1; $i<= 11; $i++)
      <tr>
        ...
        <td></td>
        @if(isset($staffDeductionElement) && $staffDeductionElement)
          @foreach($staffDeductionElement as $ttd)
          <td></td>
          @endforeach
        @endif
        <td style="background:#999;opacity:0.9;"></td>
    ...
    <td align="right"><strong>{{ number_format($pectotal, 2, '.', ',') }}</strong></td>
     <td align="right"><strong>{{ number_format($pectotalFG, 2, '.', ',') }}</strong></td>
    <td style="background:#999;opacity:0.9;" align="right"><strong>{{ number_format(($bstotal + $e_arrearstotal + $e_otherstotal + $pectotal + $pectotalFG), 2) }}</strong></td>
    <td align="right"><strong>{{ number_format($taxtotal, 2, '.', ',') }}</strong></td>
    <td align="right"><strong>{{ number_format($pentotal, 2, '.', ',') }} </strong></td>
    <td align="right"><strong>{{ number_format($uniontotal, 2, '.', ',') }} </strong></td>
    <td align="right"><strong>{{ number_format($nhftotal, 2, '.', ',') }}</strong></td>
    ...
    @endif
    <td  align="right"><strong>{{ number_format(($deducttotal + $totalDeductAmount), 2, '.', ',') }}</strong></td>
    {{-- <td style="background:#999;opacity:0.9;" align="right"><strong>{{ number_format((($netpaytotal + $deducttotal + $totalDeductAmount)-($pectotal + $pectotalFG)) - ($deducttotal + $totalDeductAmount), 2) }}</strong></td> --}}
     <td style="background:#999;opacity:0.9;" align="right"><strong>{{ number_format($totalNetEmolu, 2, '.', ',') }}</strong></td>
...
<br>
            ...
            <div class="col-md-12 text-center">PAYROLL SUMMARY VOUCHER - JUSTICE</div>

            <div class="row">
            <p><strong> MINISTRY/DEPARTMENT P.V. No. SCN/PE/_______________  </strong> <strong>    &nbsp;&nbsp; {{ isset($year) ? $year : '' }}___ </strong></p>

            <div class="col-xs-3">
            <table class="table table-v" border="1">
            ...
            <td> {{ number_format($earntotal - ($pectotal + $pectotalFG), 2, '.', ',') }} </td>
            </tr>
            <tr>
            <td>Month Basic Allowance for the month to</td>
            <td>{{ number_format(($pectotal + $pectotalFG), 2, '.', ',') }}</td>
            </tr>
            ...
            <tr>
              <td>NHF</td>
              <td>{{ number_format($nhftotal, 2, '.', ',') }}</td>
            </tr>
            ...
            @if(isset($staffDeductionElement) && $staffDeductionElement)
              @foreach($staffDeductionElement as $elementDeduct)
                  <tr>
                    <td>{{ucfirst($elementDeduct->description)}}</td>
                    @php $deducttotal += $elementDeduct->deductionAmount; @endphp
                    <td>{{ number_format(($elementDeduct->deductionAmount), 2, '.', ',') }}</td>
                  </tr>
              @endforeach
            @endif
            {{-- <tr>
           ...
           <!-- <td>HEAD___ SUBHD___</td>
            ...
            <td>&nbsp;&nbsp;&nbsp;<br/><br/></td>
            <td>&nbsp;&nbsp;&nbsp;<br/>{{ isset($month) ? $month : '' }}/{{ isset($year) ? $year : '' }}<br/></td>
            <td>&nbsp;&nbsp;&nbsp;<br/>ABUJA<br/></td>
            </tr>
            </table>
            </p>
            ...
            <p>That their employment at the rate of salary shown was duly authorised by _______________________ <br/> the amount of __________________________________ <br> __________________________________ <br> __________________________________ Naira ___________Kobo may be paid under the Heads and items shown.</p>
            <p>______________________________ Signature</p>
            <p>______________________________Title</p>
            ...
            <div class="text-center" style="width: 1500px;"><strong>AMOUNT OF CHEQUE PAYABLE &#8358</strong> <span style="border-bottom-style: dotted;"> {{ number_format($totalNetEmolu, 2, '.', ',') }} </span> &nbsp;<strong>AMOUNT IN WORDS</strong>
              <span style="border-bottom-style: dotted;" id="resultInWords"></span>.</div>
...
<script type="text/javascript">
  var amount = "";
  var amount = "{{ number_format($totalNetEmolu, 2, '.', ',') }}";
  ...
  words = word1;
  document.getElementById('resultInWords').innerHTML = words.toUpperCase();
  }
