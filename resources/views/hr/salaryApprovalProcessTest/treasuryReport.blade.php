...
<table class="" border="0" align="left" cellpadding="0" cellspacing="0" style="margin-bottom:30px; margin-top:20px;" >
  ...
  <td class ="text-right"  <?php $totalTenPer += $tenPercent; ?> >{{number_format($tenPercent, 2, '.', ',')}}</td>
  <td class ="text-right"  <?php $totalCont += $list->Vpara; ?> >{{number_format($list->Vpara, 2, '.', ',')}}</td>
  <td class ="text-right"  <?php $totalPen += $list->Vpara + $tenPercent; ?> >{{number_format($list->Vpara + $tenPercent, 2, '.', ',')}}</td>
  @else
