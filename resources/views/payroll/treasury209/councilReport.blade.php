...
<script type="text/javascript">
        function Export() {
            $("#tableData").table2excel({
                filename: "{{$selectedMonth}}_{{$year}}_TAX.xls"
            });
        }
    </script>
...
<table class="" border="0" align="left" cellpadding="0" cellspacing="0" style="margin-bottom:30px; margin-top:20px;" >
  ...
  @if($reportType == 'TAX')
  <tr><th>{{$payeeCaption->address}} DEDUCTION FOR THE MONTH OF {{strtoupper($selectedMonth)}}, {{strtoupper($selectedYear)}}</th></tr>

  
  @else
  <tr><th> {{strtoupper($Tr2019Head)}} DEDUCTION FOR THE MONTH OF {{strtoupper($selectedMonth)}}, {{strtoupper($selectedYear)}} {{strtoupper($currentstate->state ?? '')}}, {{strtoupper($division->division ?? '')}}</th></tr>
  @endif
  <tr><th></th></tr>
  ...
  <td class ="text-right"  <?php $totalBs += $list->Bs; ?> >{{number_format($list->TAX, 2, '.', ',')}}</td>
  ...
  <td></td>
    ...
    <td class ="text-right" align="right"><strong>{{number_format($totalTax, 2, '.', ',')}}</strong></td>
 ...
 <script type="text/javascript">
	var amount = "";
	var amount = "{{number_format($totalTax, 2, '.', ',')}}";
