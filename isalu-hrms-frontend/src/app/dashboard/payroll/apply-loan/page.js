...
export default function ApplyLoanPage() {
  ...
  const filteredLoans = loans.filter(l => {
    ...
    const fileMatch = l.fileNo?.toLowerCase().includes(searchQuery.toLowerCase());
    const typeMatch = l.loan_type?.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const canHodAct = userCtx.isHod || userCtx.isSuperAdmin || userCtx.isAdminStaff;
  const canAuditAct = userCtx.isAuditStaff || userCtx.isSuperAdmin || userCtx.isAdminStaff;
  const canAdminAct = userCtx.isAdminStaff || userCtx.isSuperAdmin;
