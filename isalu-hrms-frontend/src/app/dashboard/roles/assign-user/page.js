...
export default function AssignUserRolePage() {
  ...
  const fetchAssignments = useCallback(async (currentPage = 1, query = '') => {
    const headers = buildHeaders();
    try {
  ...
  useEffect(() => {
    fetchMetadata();
    fetchAssignments(1, '');
  }, [fetchMetadata, fetchAssignments]);

  // Handle page/search change
  useEffect(() => {
    fetchAssignments(page, searchQuery);
  ...
  const handleAssignSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        setSelectedUser('');
        setSelectedRole('');
        fetchAssignments(1, searchQuery); // Reload and go to first page
      } else {
