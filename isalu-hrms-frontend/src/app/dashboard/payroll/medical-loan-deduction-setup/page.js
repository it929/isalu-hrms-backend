...
export default function MedicalLoanDeductionSetupPage() {
  // UI & Loading States
  ...
  const fetchSetups = useCallback(async (silent = false) => {
    ...
    const headers = buildHeaders();
    try {
      const res = await axios.get(`${API_BASE}/payroll/medical-loan-deduction-setups`, { headers });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_BASE}/payroll/medical-loan-deduction-setups`, payload, {
        headers: buildHeaders()
      });

      if (res.data.status === 'success') {
        showToast(res.data.message || 'Medical loan configuration saved successfully.');
        handleClearForm();
        fetchSetups(true);
      } else {
  ...
  const handleToggleStatus = async (id) => {
    try {
      const res = await axios.post(`${API_BASE}/payroll/medical-loan-deduction-setups/toggle/${id}`, {}, {
        headers: buildHeaders()
      });
      if (res.data.status === 'success') {
  ...
  const handleDelete = (id) => {
    setConfirmAction({ type: 'delete', id, label: 'Delete Medical Loan Setup' });
  };
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (type === 'delete') {
        const res = await axios.delete(`${API_BASE}/payroll/medical-loan-deduction-setups/${id}`, {
          headers: buildHeaders()
        });
        if (res.data.status === 'success') {
  ...
  const uploadFile = async (file) => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/payroll/medical-loan-deduction-setups/import`, formData, {
        headers: {
          ...buildHeaders(),
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.2fr) minmax(0, 0.8fr)', gap: '1.5rem', marginBottom: '1.5rem' }}>
          ...
          <div className={styles.card} style={{ marginBottom: 0 }}>
            <div className={styles.cardHeader}>
              <h2 className={styles.cardTitle}>{editSetupId ? 'Modify Medical Loan Setup' : 'Create Medical Loan Setup'}</h2>
            </div>
            ...
            <div className={styles.cardBody} style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              ...
              <div style={{ marginTop: '1.25rem', fontSize: '0.825rem', color: '#64748b', lineHeight: '1.4' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.35rem' }}>
                  ...
                  <a
                    href={`${API_BASE}/payroll/medical-loan-deduction-setups/template`}
                    download="medical_loan_import_template.csv"
                    style={{
        ...
        <div className={styles.cardBody} style={{ padding: 0 }}>
          {loading ? (
            ...
            <div className={styles.emptyState}>
              ...
              <p>No medical loan configurations found.</p>
            </div>
          )}
          ...
          <div className={styles.modalBox}>
            <div className={styles.confirmBox}>
              ...
              <p className={styles.confirmMsg}>
                Are you sure you want to delete this medical loan configuration? This action cannot be undone.
              </p>
