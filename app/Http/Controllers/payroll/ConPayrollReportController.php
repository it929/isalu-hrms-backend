...
class ConPayrollReportController extends ParentController
{
    ...
    public function create()
    {
        ...
        return view('payroll.payrollReport_con.index', $data);
    }
    ...
    public function payrollComments13022026(Request $request)
    {
        ...
        // ->whereRaw('UPPER(tblsalary_comments.month) = ?', [strtoupper($month)])
        ...
        return view('payroll.payrollReport_con.payrollComments', $data);
    }


    //view comments on payroll
    public function payrollComments(Request $request)
    {
        // $division, $year, $month
        $division = $request->division;
        $year = $request->year;
        ...
        // Updated query to include attachment field
        $data['allcomments'] = DB::table('tblsalary_comments')
            ->leftjoin('users', 'users.id', '=', 'tblsalary_comments.by_who')
            ->where('tblsalary_comments.year', '=', $year)
            ->where('tblsalary_comments.month', $month)
            ...
            ->select(
                'users.name',
                'tblsalary_comments.divisionID as divisionID',
                'tblsalary_comments.comment',
                'tblsalary_comments.updated_at',
                'tblsalary_comments.attachment'  // ADD THIS - include attachment field
            )
            ->get();
    ...
    public function Retrieve(Request $request, $division = null, $year = null, $month = null)
    {
        ...
        $data['allEarnDeduction'] = collect(DB::table('tblotherEarningDeduction')
        ...
        $selectedMonth = Carbon::createFromDate($year, date('m', strtotime($month)), 1);

        $retirementStaff = DB::table('tblper')
            ->select('fileNo', 'dob', 'appointment_date', 'surname', 'first_name', 'othernames')
            ->get()
            ->map(function ($staff) use ($selectedMonth) {

                if (!$staff->dob && !$staff->appointment_date) {
                    return null;
                }

                $retireByAge = $staff->dob
                    ? Carbon::parse($staff->dob)->addYears(60)
                    : null;

                $retireByService = $staff->appointment_date
                    ? Carbon::parse($staff->appointment_date)->addYears(35)
                    : null;

                // get earliest retirement date
                $retirementDate = collect([$retireByAge, $retireByService])
                    ->filter()
                    ->sort()
                    ->first();
                ...
                // check if retirement is in selected month/year
                if (
                    $retirementDate->year != $selectedMonth->year ||
                    $retirementDate->month != $selectedMonth->month
                ) {
                    return null;
                }

                // check active salary
                $isInPayment = DB::table('tblpayment_consolidated')
                    ->where('fileNo', $staff->fileNo)
                    ->exists();
                ...
                $alreadyListed = DB::table('tblstaff_for_retirement')
                    ->where('fileNo', $staff->fileNo)
                    ->exists();
        ...
        $data['retirementStaff'] = $retirementStaff;

        // dd($data['payroll_detail']);
        return view('payroll.payrollReport_con.summary', $data);
        //===================== query ends ====================
    ...
    public function DynamicEaringDeduction($listOfStaff = [], $year = null, $month = null, $division = null, $bankID = null)
    {
        ...
        $data['staffEarnElement'] = DB::table('tblotherEarningDeduction')
            ...
            ->where('tblotherEarningDeduction.amount', '>', 0.0099)
    ...
    public function BulkPayRoll(Request $request)
    {
        ...
        return view('payroll.payrollReport.bulkPayroll', $data);
    }


    public function paidStaffSearch01()
    {
        $data['courts'] = DB::table('tbl_court')->get();
        $courtSessionId = session('anycourt');
        $data['CourtInfo'] = $this->CourtInfo();
        ...
        return view('payroll.payrollReport_con.paid-staff-search', $data);
    }

    public function getPaidStaff01(Request $request)
    {
        $request->validate([
            'division' => 'required',
            'year' => 'required',
        ...
        $query = DB::table('tblpayment_consolidated as pc')
            ->join('tblper as p', 'pc.fileNo', '=', 'p.fileNo')  // Join using file number instead of staffid
            ->where('pc.payment_status', '=', 1)
            ->where('pc.year', '=', $request->year)
            ->where('pc.month', '=', strtoupper($request->month))
            ->select(
                'p.ID',
                'p.fileNo',
                'p.surname',
                ...
                'p.grade',
                'p.step',
                'pc.*'  // This will include all payment consolidated fields
            );

        // Apply division filter
        if ($request->has('division') && !empty($request->division)) {
            if (Auth::user()->is_global == 1) {
                $query->where('pc.divisionID', '=', $request->division);
            } else {
                $query->where('pc.divisionID', '=', Auth::user()->divisionID);
            }
        }

        $data['paidStaff'] = $query->get();
        $data['searchParams'] = $request->all();

        return view('payroll.payrollReport_con.paid-staff-results', $data);
    }

    public function paidStaffSearch()
    {
        $data['courts'] = DB::table('tbl_court')->get();
        $courtSessionId = session('anycourt');
        $data['CourtInfo'] = $this->CourtInfo();
        ...
        $data['employmentTypes'] = DB::table('tblemployment_type')
            ->where('active', 1)
    ...
    public function getPaidStaff(Request $request)
    {
        $request->validate([
            'division' => 'required',
            'year' => 'required',
        ...
        $query = DB::table('tblpayment_consolidated as pc')
            ->join('tblper as p', 'pc.fileNo', '=', 'p.fileNo')
            ->where('pc.payment_status', '=', 1)
            ->where('pc.year', '=', $request->year)
            ->where('pc.month', '=', strtoupper($request->month))
            ->select(
        ...
        // Independent filters - each one works on its own
        // Employment type filter (independent)
        if ($request->filled('employment_type')) {
            $query->where('pc.employment_type', '=', $request->employment_type);
        }

        // Grade filter (independent)
        if ($request->filled('grade')) {
            $query->where('p.grade', '=', $request->grade);
        }

        // Step filter (independent)
        if ($request->filled('step')) {
            $query->where('p.step', '=', $request->step);
        }

        // File number filter (independent)
        if ($request->filled('fileNo')) {
            $query->where('p.fileNo', 'LIKE', '%' . $request->fileNo . '%');
        }

        $data['paidStaff'] = $query->get();
        $data['searchParams'] = $request->all();

        // Get employment types for displaying names in results
        $data['employmentTypes'] = DB::table('tblemployment_type')
            ->where('active', 1)
            ->pluck('employmentType', 'id');
    ...
    public function arrearsOearn($court, $fileNo, $year, $month)
    {
        ...
        $data['oarrears'] = DB::table('tblarrears')
            ->select('fileNo', 'month', 'year', 'oldGrade', 'OldStep', 'newGrade', 'newStep', 'newBasic', 'oldBasic', 'oldTax', 'newTax', 'oldPeculiar', 'newPeculiar', 'oldPeculiarFG', 'newPeculiarFG', 'oldPension', 'newPension', 'oldNhf', 'newNhf', 'oldUnionDues', 'newUnionDues', 'dueDate', 'date')
            ->where('month',     '=', $month)
            ->where('year',      '=', $year)
            ->where('courtID',   '=', $court)
            ->where('staffid',      '=', $fNo)
        ...
        $data['oarrearsReason'] = DB::table('tblstaff_for_arrears')
            ->join('tbldivision', 'tbldivision.divisionID', 'tblstaff_for_arrears.divisionID')
            ->select('arrears_type', 'division')
            ->where('month_payment',     '=', $month)
            ->where('year_payment',      '=', $year)
            ->where('tblstaff_for_arrears.courtID',   '=', $court)
            ->where('staffid',      '=', $fNo)
            ->first();
        ...
        $data['overDues'] = DB::table('tblarrears_overdue')
            ...
            ->join('tbldivision', 'tbldivision.divisionID', 'tblstaff_for_arrears_overdue.divisionID')
        ...
        return view('payroll.payrollReport_con/otherArrears', $data);
    }
    ...
    public function arrearsOearnTest($court, $fileNo, $year, $month)
    {
        ...
        //dd( $data['overDue']);
        return view('payroll.payrollReport_con/otherArrears_17_06_2019', $data);
    }
    ...
    public function payrollBreakdown()
    {
        ...
        return view('payroll.payrollReport_con.payrollBreakdown', $data);
    }
    ...
    public function payrollBreakdownReport(Request $request)
    {
        ...
        if ($division == '' &&  $bankGroup != '' &&  $bankID != '') {
            ...
            $data['payroll_detail'] = DB::table('tblpayment_consolidated')
                ...
                ->orderBy('step', 'DESC')
                ->get();
            return view('payroll.payrollReport_con.payrollReportBreakdown', $data);
        ...
        return view('payroll.payrollReport.payrollReportBreakdown', $data);
    }
    ...
    public function CompareEarningOLD(Request $request)
    {
    ...
    and ((`year`='$year1' and `month`='$month1' and $qbank) or (`year`='$year2' and `month`='$month2' and $qbank)) and rank<>2 group by `staffid` order by tblbanklist.bank ");
        ...
        // dd($data['record']);
        return view('payroll.payrollReport_con.compare_earning', $data);
    }
    ...
    public function CompareEarning(Request $request)
    {
        ...
        if ($data['CourtInfo']->courtstatus == 0) {
            $request->merge(['court' => $data['CourtInfo']->courtid]);
        }
        if ($data['CourtInfo']->divisionstatus == 0) {
            $request->merge(['division' => $data['CourtInfo']->divisionid]);
            $division = $data['CourtInfo']->divisionid;
        }
        ...
        if (Auth::user()->is_global != 1) {
            ...
            $data['allbanklist'] = DB::table('tblbanklist')->orderBy('bank', 'ASC')->get();
        }

        $query = DB::table('tblpayment_consolidated')
            ->leftJoin('tblbanklist', 'tblbanklist.bankID', '=', 'tblpayment_consolidated.bank')
            ->select('tblpayment_consolidated.*', 'tblbanklist.bank as bankname')
            ->where('divisionID', $division)
            ->where('rank', '<>', 2)
            ->where(function ($q) use ($year1, $month1, $year2, $month2, $bankID) {
                $q->where(function ($q1) use ($year1, $month1, $bankID) {
                    $q1->where('year', $year1)
                        ->where('month', $month1);
                    if (!empty($bankID)) {
                        $q1->where('tblpayment_consolidated.bank', $bankID);
                    }
                })->orWhere(function ($q2) use ($year2, $month2, $bankID) {
                    $q2->where('year', $year2)
                        ->where('month', $month2);
                    if (!empty($bankID)) {
                        $q2->where('tblpayment_consolidated.bank', $bankID);
                    }
                });
            })
            ->groupBy('tblpayment_consolidated.staffid')
            ->orderBy('tblbanklist.bank', 'ASC');

        $affectedStaff = $query->get();

        $remarkList = DB::table('compare_earning_remarks')
            ->where('month', $month1)
            ->where('year', $year1)
            ->where('month2', $month2)
            ->where('year2', $year2)
        ...
        foreach ($affectedStaff as $v) {
            $total1 = ($sp != '1')
                ? $this->ThisMonthEarn($v->staffid, $year1, $month1)
                : $this->ThisMonthEarnExludeSP($v->staffid, $year1, $month1);

            $total2 = ($sp != '1')
                ? $this->ThisMonthEarn($v->staffid, $year2, $month2)
                : $this->ThisMonthEarnExludeSP($v->staffid, $year2, $month2);
            ...
            if (round($diff, 2) != 0.00) {
                $data['record'][] = [
                    ...
                    'Banks'   => $v->bankname,
                    'year'    => $v->year,
                    'staffid' => $v->staffid, // ✅ Use lowercase key consistently
                    'net1'    => $total1,
                    'net2'    => $total2,
                    'diff'    => $diff,
                ];
            ...
            $data['getStaffRemark'][$v->staffid] = $remarkList[$v->name] ?? '';
        }
    ...
    public function curDivisionOLD($userId)
    {
    ...
    public function ComparePension(Request $request)
    {
        ...
        //dd( $data['record']);
        return view('payroll.payrollReport_con.compare_pension', $data);
    }
    ...
    public function newPayrollIndex()
    {
        ...
        // dd($data);
        return view('payroll.payrollReport_con.newPayrolIndex', $data);
    }
    ...
    public function newPayrollReport(Request $request)
    {
        ...
        if ($bankID == '') {
            ...
            return view('payroll.payrollReport_con.newPayrollReport', $data);
        } else {
            $data['payroll_detail'] = DB::table('tblpayment_consolidated')
                //->where('tblpayment_consolidated.month',     '=', $month)
                ->whereIn('tblpayment_consolidated.month', $monthsToSearch)
